...
class Scaling(nn.Module):
    """ a module that scales each input by a learnable scalar """
    def __init__(self, in_features: int, init_value: float = 1.0):
        super().__init__()
        self.weight = nn.Parameter(T.ones(in_features)*init_value)
...
class MLPBlock(nn.Module):
    ...
    def __init__(
        ...
        for n in range(n_layers):
            ...
            if n == n_layers - 1:
                self.block.append(get_act(act_o))
            else:
        ...
        if scale_residual is not None:
            self.scale_residual_layer = Scaling(inpt_dim, scale_residual)
        else:
            self.scale_residual_layer = nn.Identity()
    ...
    def forward(self, inpt: T.Tensor, ctxt: T.Tensor | None = None) -> T.Tensor:
        ...
        if self.do_res=="adjust":
            if self.inpt_dim < self.outp_dim:
                temp[:, :self.inpt_dim] += self.scale_residual_layer(inpt)
            else:
                temp += self.scale_residual_layer(inpt)[:, :self.outp_dim]
        elif self.do_res:
            temp = temp + self.scale_residual_layer(inpt)

...
class Scaling(nn.Module):
    ...
    def __init__(self, in_features: int, init_value = 1.0):
        ...
        if isinstance(init_value, float):
            ...
            vector = np.array([1] * half_n + [-1] * half_n, dtype=np.float32)
            np.random.shuffle(vector)
            init_vector = T.tensor(vector)
            self.weight = nn.Parameter(init_vector)
        else:
            raise ValueError("init_value must be a float or something good")

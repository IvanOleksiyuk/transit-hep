...
class TwinTURBOv2(LightningModule):
	...
	def _shared_step(self, sample: tuple, _batch_index = None) -> torch.Tensor:
		v1, v2 = sample
		# Direc pass
		...
		loss_attractive = -self.asim_alpha*cosine_similarity(v1, v2[torch.randperm(len(v2))])
		loss_repulsive = torch.abs(self.asim_beta*cosine_similarity(v1, v2))
...
class TwinTURBOVICreg(LightningModule):
	...
	def _shared_step(self, sample: tuple, _batch_index = None, step_type="none") -> torch.Tensor:
		...
		if step_type=="train":
			self.loss_balabcing(self, loss_repulsive)
		self.log("train/l_atr_weight", self.loss_weights.loss_attractive)	
		self.log("train/l_rep_weight", self.loss_weights.loss_repulsive)
		return total_loss
